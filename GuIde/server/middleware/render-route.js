...
function coReadFile(filePath) {
  return new Promise(function(resolve, reject) {
    fs.readFile(filePath, 'utf-8', function (err, data) {
      if (err) {
        reject(err);
      } else {
        resolve(data.replace('${mount}', '<div id="mount"></div>'));
      }
    });
  });
  /*
  return readFileThunk(done) {
    fs.readFiel(filePath, function (err, data) {
      if (err) {
        done(err);
      } else {
        done(null, data);
...
module.exports = function makeRenderRouteMiddleware (middleware) {
  return function *renderRoute(next) {
    if (this.request.url === '/old') {
      const htmls = yield render(this.request.url);
      this.body = `
      <!DOCTYPE html>
      ...
      let initialState = { gudmarks: { techcuz: [{ cate: 'angular', links: [{url: '2', name: 'angular react'}] }] } };
    ...
    function *renderRouteMiddleware () {
      let initialState = { techcuz: [] };
      if (typeof middleware === 'function') {
        initialState = yield middleware.call(this);
      }
      try {
        this.body = yield render(this.request.url);
      } catch (e) {
        yield* next;
      }
    }
